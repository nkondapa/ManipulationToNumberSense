...
from plotting.action_prediction_psychometric_curve_plots import *
...
FOLDER_PATH = FIGURE_OUTPUT_FOLDER + '/action_pred_perception_figures/'
os.makedirs(FOLDER_PATH, exist_ok=True)

d2 = {'n_obj': 30, 'dsize': 150, 'dataset_name': 'random_contrast_random_sizes_test_set', 'set_num': 1}
main_exp = 'model2_0'
model = AnalyzeModel.load_model(main_exp, 'final.pt').eval().cpu()


def action_classification(output1, output2):
    cat_out = torch.cat([output1, output2], dim=1)
    out = model.nonlinear(cat_out)
    out = model.fc1(out)
    out = model.fc2(out)
...
embedding_dict = {}
for un_count in np.unique(main_embedding_counts):
    mask = main_embedding_counts == un_count
    embedding_dict[un_count] = main_embeddings[mask]
...
generate_action_prediction_psychometric_curve(embedding_dict, 16, list(range(0, 30)), action_classification, ax)
...
fig.savefig(FOLDER_PATH + 'proportion_more_figure.png')
